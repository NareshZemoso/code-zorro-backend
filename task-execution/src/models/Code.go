...
func (code *Code) SaveToFile(nameChannel *chan string){
	...
	os.Mkdir("codes",0755)
	f,err:=ioutil.TempFile("codes",strconv.FormatInt(time.Now().Unix(),10)+"*"+ext)
	if err!=nil {
		panic(err.Error())
	}
...
func (code *Code) Run() (string,bool) {
	...
	codeOutput := make(chan string)
	go RunSpecificCode(code, fname, &codeOutput)
	res := <-codeOutput
	var runStatus bool
